...
- [ ] 编写 Vitest 单元测试
  - [ ] 通用组件测试 (Navbar, Sidebar, JobCard 等)
  - [ ] 状态管理测试 (userStore, jobStore 等)
  - [ ] 工具函数测试 (request.ts, scroll-to.ts 等)
- [ ] 端到端测试（Playwright）
  - [ ] 认证流程测试（登录、注册）
  - [ ] 学生端关键流程测试（浏览职位、申请职位）
  - [ ] 企业端关键流程测试（发布职位、管理申请）
...
- tests/ 下测试用例（待完成）
  ...
  - tests/stores/ 目录（状态管理测试）
  - tests/utils/ 目录（工具函数测试）
- e2e/ 下端到端测试用例（待完成）
...
- 文档结构已完善，但测试部分仍需补充
- 未完成功能清单将作为后续开发的重要参考
- 测试用例应与功能开发同步进行，确保代码质量
- 定期运行测试并生成覆盖率报告，及时发现问题
...
项目当前完成度约为 **80%**，主要功能模块已实现基本骨架，但还有一些功能细节需要完善，测试用例开发也需要同步进行。
